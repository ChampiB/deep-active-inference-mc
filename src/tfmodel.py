...
class ModelDown(tf.keras.Model):
    def __init__(self, s_dim, pi_dim, tf_precision, precision, colour_channels, resolution):
        ...
        self.qs_net = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(256, activation=tf.nn.relu, kernel_initializer='he_uniform'),
            tf.keras.layers.Dropout(0.5),
            tf.keras.layers.Dense(s_dim + s_dim)
        ])  # No activation

        # Create decoder network.
        last_strides = 2 if self.resolution == 64 else 1
    ...
    @tf.function
    def encoder(self, o):
        return tf.split(self.qs_net(o), num_or_size_splits=2, axis=1)
...
class ActiveInferenceModel:
    def __init__(self, s_dim, pi_dim, gamma, beta_s, beta_o, colour_channels=1, resolution=64):
        ...
        self.pi_one_hot = tf.Variable(initial_value=np.identity(4), trainable=False, dtype=self.tf_precision)
    ...
    def save_all(self, folder_chp, script_file=""):
    ...
    def check_reward(self, o):
        ...
        raise Exception("In tfmodel.py: Unknown resolution in ActiveInferenceModel.check_reward().")
    ...
    def imagine_future_from_o(self, o0, pi):
        ...
        return self.model_down.decoder(ps1)
    ...
    def calculate_G_repeated(self, o, pi, steps=1, calc_mean=False, samples=10):
        ...
        qs0_mean, _, qs0 = self.model_down.encoder_with_sample(o)
    ...
    def calculate_G(self, s0, pi0, samples=10):
        ...
        for _ in range(samples):
            ...
            _, _, qs1_logvar = self.model_down.encoder_with_sample(po1)
    ...
    def mcts_step_simulate(self, starting_s, depth, use_means=False):
        ...
        if 1 < depth:
            s0[1] = ps1_mean_new[0].numpy() if use_means else ps1_new[0].numpy()
        ...
        for t in range(1, depth):
            ...
            if t+1 < depth:
                s0[t+1] = ps1_mean_new[0].numpy() if use_means else ps1_new[0].numpy()
